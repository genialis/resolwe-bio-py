...
class BaseField:
    ...
    def _check_writeable(self, instance: "BaseResource"):
        ...
        loading = getattr(instance, "_data_origin_status") == DataSource.SERVER
    ...
    def changed(self, instance: "BaseResource"):
        ...
        if (
            instance.id is None
            and self._access_type in access_types
            and status != FieldStatus.UNSET
        ):
            return True

        status = self.status(instance)
        # No changes when the field is not set.
        if status == FieldStatus.UNSET:
            return False

        original = getattr(instance, self._original_attribute_name)
        current = getattr(instance, self._value_attribute_name)
        # Consider two lazy fields as same only when they are the same object.
        if self._is_lazy(original):
            if self._is_lazy(current):
                return original != current
            return True

        if status == FieldStatus.SET:
            # Both original and current are set to a Python object.
            return original != current
        else:  # Original is set, current is lazy.
            return True
    ...
    def __set__(self, instance: "BaseResource", value):
        ...
        if self.status(instance) == FieldStatus.UNSET:
            setattr(instance, self._original_attribute_name, copy.deepcopy(value))
        setattr(instance, self._value_attribute_name, value)
...
class FloatField(BaseField):
    ...
    def _checks_before_set(self, instance: "BaseResource", value):
        """Perform all checks before setting the value."""
        super()._checks_before_set(instance, value)

        if value is not None and not isinstance(value, float):
            raise ValueError(f"Value for '{self.public_name}' must be a float.")
...
class BooleanField(BaseField):
    ...
    def _checks_before_set(self, instance: "BaseResource", value):
        ...
        if value is not None and not isinstance(value, bool):
            raise ValueError(f"Value for '{self.public_name}' must be a boolean.")
...
class DictResourceField(BaseField):
    ...
    def _to_python_single(self, value, instance=None):
        ...
        if isinstance(value, dict):
            data_source = getattr(instance, "_data_origin_status")
            return self.Resource(
                resolwe=instance.resolwe, data_origin_status=data_source, **value
            )
        return value
