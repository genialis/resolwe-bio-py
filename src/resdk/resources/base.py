...
class BaseResource:
    ...
    def __init__(self, resolwe, data_origin_status=DataSource.SERVER, **model_data):
        """Initialize attributes."""
        self._fields = self._find_fields()
        self._original_values = {}
        self._data_origin_status = data_origin_status
        self.api = operator.attrgetter(self.endpoint)(resolwe.api)
        self.resolwe = resolwe
        self.logger = logging.getLogger(__name__)

        if model_data:
            self._update_fields(model_data, data_origin_status=data_origin_status)
        self._data_origin_status = DataSource.USER
    ...
    def _get_fields(self, *access_types):
    ...
    def _update_fields(self, payload, data_origin_status=DataSource.USER):
        ...
        try:
            ...
            self._data_origin_status = data_origin_status
            if data_origin_status == DataSource.USER:
            ...
            for field_name, field in self._fields.items():
                ...
                if field.server_field not in payload:
                    continue
                json_value = payload.get(field.server_field)
                # The field itself must handle the deserialization.
                python_value = field.to_python(json_value, self)
    ...
    def update(self):
        ...
        self._update_fields(
            self.api(self.id).get(), data_origin_status=DataSource.SERVER
        )
    ...
    def save(self):
        ...
        if payload := {
            ...
            self._update_fields(api_call(payload), data_origin_status=DataSource.SERVER)
...
class BaseResolweResource(BaseResource):
    ...
    # contributor = DictResourceField(resource_class=User)
